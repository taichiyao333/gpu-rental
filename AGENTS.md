...
│   │   ├── mypage/        ← マイページ
│   │   └── downloads/     ← gpurental-agent.exe（38MB, git除外）
...
│   └── monitor.js         ← アップタイム監視
└── gpurental-ai-kit/      ← Pythonエージェント（EXE化）
...
| **Cloudflare Tunnel** | localhost:3000 → gpurental.jp |
| **FTPホスト** | `mdl-japan.sakura.ne.jp` |
| **FTPパス** | `/www/gpurental/` |
| **DBファイル** | `./data/gpu-rental.db` (sql.js, SQLite) |
...
### エージェント接続フロー
1. プロバイダーが `gpurental-agent.exe` を起動
