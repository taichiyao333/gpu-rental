...
const CONFIG = {
    ...
    smtp: {
        ...
        user: process.env.SMTP_USER || 'noreply@gpurental.jp',
    ...
    checks: [
        {
            id:   'api-health',
            name: 'APIサーバー (Health)',
            url:  'https://gpurental.jp/api/health',
            method: 'GET',
        ...
        },
        {
            id:   'provider',
            name: 'プロバイダーポータル',
            url:  'https://gpurental.jp/provider/',
            method: 'GET',
            expect: { status: 200 },
            critical: false,
        },
        {
            id:   'diagnose',
            name: '接続診断ページ',
            url:  'https://gpurental.jp/provider/diagnose.html',
            method: 'GET',
            expect: { status: 200 },
            critical: false,
        },
        {
            id:   'agent-download',
            name: 'エージェントDL',
            url:  'https://gpurental.jp/downloads/gpurental-agent.exe',
            method: 'HEAD',
            expect: { status: 200 },
            critical: false,
        },
...
function httpCheck(check) {
    return new Promise((resolve) => {
        ...
        const req = lib.request({
            ...
            headers:  { 'User-Agent': 'GPURental-Monitor/1.0' },
...
function sendAlert(check, result) {
    ...
    const subject = `🔴 [GPURental] ${check.name} がダウンしています`;
    const body = [
        `監視アラート: ${check.name}`,
        `URL: ${check.url}`,
        `エラー: ${result.error}`,
    ...
    const cmds = [
        ...
        'DATA',
        `From: GPURental Monitor <${CONFIG.smtp.user}>\r\nTo: ${CONFIG.smtp.to}\r\nSubject: ${subject}\r\n\r\n${body}\r\n.`,
        'QUIT',
    ];
    socket.on('data', () => {
