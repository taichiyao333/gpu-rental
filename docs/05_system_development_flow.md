...
【インターネット側】
  エンドユーザー
       │  HTTPS (S3 Presigned URL or API Gateway)
       ▼
...
| ロール | 台数 | 役割 |
|--------|------|------|
| ロードバランサー | 2台 (冗長) | クライアント→Filerへの振り分け |
...
Phase 3: レンダリングエンジン開発 ──────► 次フェーズ
  └─ ジョブキュー / GPU割当 / A6000最適化

Phase 4: Webプラットフォーム開発 ───────► 次フェーズ
  └─ UI / API / 課金 / ユーザー管理
...
Phase 7: アーカイブ・長期運用 ──────────► 最終フェーズ
  └─ HDD 1PBアーカイバー / ライフサイクル管理
```
...
ユーザーがS3にアップロードしたファイルを、自動でデータセンターのSeaweedFSに取り込む。
