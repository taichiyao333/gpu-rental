 ...
 * gpurental.jpに外向きWebSocket接続を張り、SSHトンネルを提供する。
 * 
 * ★ ポート開放不要 ★
 * 
 * Usage:
 ...
 *   AGENT_EMAIL   = user@example.com  (GPURentalアカウントのメール)
 *   SSH_HOST      = 127.0.0.1
        ...
        try {
            ...
            if (!data.success) {
                ...
                if (data.error && data.error.includes('アカウントが見つかりません')) {
                    showError(
                        'アカウントが見つかりません',
                        `メールアドレス「${config.email}」で登録されたGPURentalアカウントがありません。`,
                        '先に https://gpurental.jp/portal/ でアカウントを作成してください。'
                    );
                } else {
...
function connectTunnel() {
    ...
    socket.on('tunnel:ready', (data) => {
        ...
        console.log(`  ║  ssh -p ${data.port} <username>@gpurental.jp  ║`);
        console.log('  ╠═══════════════════════════════════════════════╣');
