...
// ====== 設定 ======
const PLATFORM_URL = 'https://gpurental.jp';
const AGENT_VERSION = '1.0.0';
const CONFIG_DIR = path.join(os.homedir(), '.gpurental');
...
function registerAutoStart(exePath) {
  try {
    ...
    execSync(`reg add "${regKey}" /v "GPURentalAgent" /t REG_SZ /d ${quotedPath} /f`, {
      encoding: 'utf8'
    });
...
<title>GPURental エージェント セットアップ</title>
<style>
...
<div class="card">
  ...
  <div class="form-group">
    <label>📧 GPURentalアカウントのメールアドレス</label>
...
async function main() {
  ...
  log(`GPURental Agent v${AGENT_VERSION} 起動`);
