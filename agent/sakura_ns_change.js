...
async function main() {
  ...
  try {
    ...
    // gpurental.jpを探す
    console.log('\n📍 Step 3: gpurental.jpを検索...');
    await sleep(2000);

    // JavaScriptでドメインリストを取得
    const domainList = await page.evaluate(() => {
      const links = Array.from(document.querySelectorAll('a'));
      return links.filter(l => l.textContent.includes('gpurental') || l.href.includes('gpurental'))
                  .map(l => ({ text: l.textContent.trim(), href: l.href }));
    });
    console.log('gpurental関連リンク:', domainList);

    // ドメインをクリック
    if (domainList.length > 0) {
      await page.click(`a[href*="gpurental"], a:has-text("gpurental")`);
      await sleep(3000);
      console.log('gpurental.jpをクリック! URL:', page.url());
    } else {
      console.log('⚠️  gpurental.jpが見つかりません。');
      console.log('   Chromeウィンドウでgpurental.jpをクリックしてください。');
