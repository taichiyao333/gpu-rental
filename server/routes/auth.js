...
router.post('/register', async (req, res) => {
    ...
    if (!/[A-Z]/.test(password) && !/[0-9]/.test(password) && password.length < 12)
        return res.status(400).json({ error: 'パスワードは8文字以上、または大文字か数字を含めてください' });
    ...
    if (!captchaOk) return res.status(400).json({ error: '自動化の疑いがあります。もう一度お試しください' });
...
router.post('/forgot-password', (req, res) => {
    ...
    const db = getDb();
    // statusに関わらずメールアドレスが一致するユーザーを検索
    ...
    // テーブルが存在しない場合は exec() で作成 (migrationsで作成済みのはずだが念のため)
    try {
...
router.post('/reset-password', (req, res) => {
    ...
    console.log(`[Auth] Password reset complete for user #${reset.user_id}`);
