...
// RunPodのGPU名→GPURentalのGPU名 マッピング
const GPU_NAME_MAP = {
    ...
    return new Promise((resolve, reject) => {
        const options = {
            ...
            headers: {
                ...
                'User-Agent': 'GPURental-PriceMonitor/1.0',
            },
            timeout: 15000,
        };
...
async function runPricingSnapshot(db) {
    ...
    for (const rp of runpodPrices) {
        ...
        comparisons.push({
            runpod_name: rp.name,
            gpurental_name: gpuName,
            vram_gb: rp.vram_gb,
            runpod_price_usd: rp.price_usd_hr,
            runpod_price_jpy: rp.price_jpy_hr,
            gpurental_price_jpy: current?.price_per_hour || null,
...
if (require.main === module) {
    ...
    runPricingSnapshot(db).then(result => {
        console.log('\n=== RunPod vs GPURental Price Comparison ===');
        result.comparisons.forEach(c => {
            ...
            console.log(
                ...
                `GPURental: ¥${c.gpurental_price_jpy || 'N/A'}  ` +
